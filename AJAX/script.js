...
function openModal(data) {
  ...
  var list = document.getElementById("modalDataList");
  list.innerHTML = "";
  [
    "類型：" + (data.type === "expense" ? "支出" : "收入"),
    "金額：$ " + formatCurrency(Number(data.amount)),
    "類別：" + data.category,
    "日期：" + data.date,
    "備註：" + (data.note || "-")
...
function submitToBackend(payload, onSuccess) {
  if (!window.jQuery) {
    alert("送出失敗：缺少 jQuery，無法使用 AJAX。");
    return;
  }
  window.jQuery.ajax({
    url: "result.jsp",
    ...
    error: function () {
      alert("送出失敗：無法連線到 result.jsp，請確認是否以 JSP/Servlet 容器執行。");
    }
  });
}
